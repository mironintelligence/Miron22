        ...
        <Stagger className="grid grid-cols-1 md:grid-cols-3 gap-[1px] bg-border">
          {CARDS.map(({ num, title, desc, badges }) => (
            ...
            >
              ...
              <h3 className="font-sub font-bold text-[14px] mb-4 text-text">{title}</h3>
              <p className="font-ui text-[13px] text-muted leading-relaxed mb-5">{desc}</p>
              <div className="flex flex-wrap gap-2">
                {badges.map((b) => (
                  <span
                    key={b}
                    className="font-ui text-[10px] tracking-[0.14em] uppercase px-2.5 py-1 border border-border text-muted"
                  >
                    {b}
                  </span>
                ))}
        ...
        <div className="mt-[2px] grid grid-cols-1 md:grid-cols-3 border border-border">
          {META.map(({ label, value }, i) => (
            ...
            >
              ...
              <p className="font-ui text-[14px] text-text mt-1">{value}</p>
            </div>
          ))}
        </div>
      </div>
