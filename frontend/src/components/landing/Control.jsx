        ...
        <div className="flex flex-col mt-0">
          {TIMELINE.map(({ label, title, desc }, i) => (
            <Reveal key={label} delay={i * 0.1}>
              <div className="py-[30px] border-b border-border first:border-t grid grid-cols-[140px_1fr] gap-9 hover:pl-3.5 transition-all duration-300">
                <span className="font-ui text-[10px] tracking-[0.22em] uppercase text-gold pt-1">
                  {label}
                </span>
                <div>
                  <p className="font-ui text-[14px] text-white mb-1">{title}</p>
